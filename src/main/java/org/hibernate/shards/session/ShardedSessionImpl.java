...
public class ShardedSessionImpl implements ShardedSession, ShardedSessionImplementor,
    ...
    static List<Shard> buildShardListFromSessionFactoryShardIdMap(
        ...
        for (Map.Entry<SessionFactoryImplementor, Set<ShardId>> entry : sessionFactoryShardIdMap
                .entrySet()) {
            ...
            if (checkAllAssociatedObjectsForDifferentShards) {
                // cross shard association checks for updates are handled using
                // interceptors
                CrossShardRelationshipDetectingInterceptor csrdi = new CrossShardRelationshipDetectingInterceptor(
                        shardIdResolver);
                if (interceptorToSet == null) {
                    // no interceptor to wrap so just use the cross-shard
                    // detecting interceptor raw
                    // this is safe because it's a stateless interceptor
                    interceptorToSet = csrdi;
                } else {
                    // user specified their own interceptor, so wrap it with a
                    // decorator
                    // that will still do the cross shard association checks
                    Pair<Interceptor, OpenSessionEvent> result = decorateInterceptor(csrdi,
                            interceptor);
                    interceptorToSet = result.first;
                    eventToRegister = result.second;
                }
            } else if (interceptorToSet != null) {
                // user specified their own interceptor so need to account for
                // the fact
            ...
            Shard shard = new ShardImpl(entry.getValue(), entry.getKey(), interceptorToSet);
            list.add(shard);
    ...
    static Pair<Interceptor, OpenSessionEvent> handleStatefulInterceptor(Interceptor mightBeStateful) {
        ...
        if (mightBeStateful instanceof StatefulInterceptorFactory) {
            ...
            if (mightBeStateful instanceof RequiresSession) {
                openSessionEvent = new SetSessionOnRequiresSessionEvent(
                        (RequiresSession) mightBeStateful);
            }
        }
    ...
    static Pair<Interceptor, OpenSessionEvent> decorateInterceptor(
            final CrossShardRelationshipDetectingInterceptor csrdi, final Interceptor decorateMe) {

        Pair<Interceptor, OpenSessionEvent> pair = handleStatefulInterceptor(decorateMe);
        Interceptor decorator = new CrossShardRelationshipDetectingInterceptorDecorator(csrdi,
                pair.first);
    ...
    private Object applyGetOperation(final ShardOperation<Object> shardOp,
        ...
        return shardStrategy.getShardAccessStrategy().apply(shardIdListToShardList(shardIds),
                shardOp, new FirstNonNullResultExitStrategy<Object>(),
    ...
    public void setFlushMode(final FlushMode flushMode) {
        ...
        SetFlushModeOpenSessionEvent event = new SetFlushModeOpenSessionEvent(flushMode);
        for (Shard shard : shards) {
            if (shard.getSession() != null) {
    ...
    public void setCacheMode(final CacheMode cacheMode) {
        ...
        SetCacheModeOpenSessionEvent event = new SetCacheModeOpenSessionEvent(cacheMode);
        for (Shard shard : shards) {
            if (shard.getSession() != null) {
    ...
    public Serializable getIdentifier(final Object object) throws HibernateException {
        ...
        throw new TransientObjectException(
                "Instance is transient or associated with a defferent Session");
    }
    ...
    @Override
    public Object load(final Class clazz, final Serializable id, final LockMode lockMode)
            throws HibernateException {

        List<ShardId> shardIds = selectShardIdsFromShardResolutionStrategyData(new ShardResolutionStrategyDataImpl(
                clazz, id));
        if (shardIds.size() == 1) {
            return shardIdsToShards.get(shardIds.get(0)).establishSession()
                    .load(clazz, id, lockMode);
        } else {
            Object result = get(clazz, id, lockMode);
            if (result == null) {
                shardedSessionFactory.getEntityNotFoundDelegate().handleEntityNotFound(
                        clazz.getName(), id);
            }
            return result;
        }
    ...
    public Object load(final String entityName, final Serializable id, final LockMode lockMode)
        ...
        if (shardIds.size() == 1) {
            ...
            if (result == null) {
                shardedSessionFactory.getEntityNotFoundDelegate().handleEntityNotFound(entityName,
                        id);
            }
            return result;
        }
    ...
    public void replicate(final String entityName, final Object object,
        ...
        if (shardIds.size() == 1) {
            ...
            shardIdsToShards.get(shardIds.get(0)).establishSession()
            ...
            if (result == null) { // non-persisted object
                ...
                shardIdsToShards.get(shardId).establishSession()
                        .replicate(entityName, object, replicationMode);
            } else {
                Shard objectShard = getShardForObject(result, shardIdListToShardList(shardIds));
                evict(result);
    ...
    ShardId selectShardIdForNewObject(final Object obj) {
        ...
        log.debug(String.format("Selected shard %d for object of type %s", shardId.getId(), obj
                .getClass().getName()));
        return shardId;
    }
    ...
    private void checkForUnsupportedTopLevelSave(final Class<?> clazz) {
        ...
        if (classesWithoutTopLevelSaveSupport.contains(clazz)) {
            final String msg = String.format(
                    "Attempt to save object of type %s as a top-level object.", clazz.getName());
            log.error(msg);
    ...
    ShardId getShardIdOfRelatedObject(final Object obj) {
        ...
        for (Pair<Type, Object> pair : CrossShardRelationshipDetectingInterceptor
                .buildListOfAssociations(types, values)) {
    ...
    ShardId checkForConflictingShardId(ShardId existingShardId, final Class<?> newObjectClass,
        ...
        if (localShardId != null) {
            if (existingShardId == null) {
                ...
                final String msg = String
                        .format("Object of type %s is on shard %d but an associated object of type %s is on shard %d.",
                                newObjectClass.getName(), existingShardId.getId(), associatedObject
                                        .getClass().getName(), localShardId.getId());
                log.error(msg);
    ...
    List<Shard> determineShardsObjectViaResolutionStrategy(final Object object) {
        ...
        ShardResolutionStrategyData srsd = new ShardResolutionStrategyDataImpl(object.getClass(),
                id);
        List<ShardId> shardIds = selectShardIdsFromShardResolutionStrategyData(srsd);
    ...
    public void persist(final String entityName, final Object object) throws HibernateException {
        ...
        Preconditions.checkNotNull(shardId);
        setCurrentSubgraphShardId(shardId);
        log.debug(String.format("Persisting object of type %s to shard %s", object.getClass(),
                shardId));
    ...
    public Criteria createCriteria(final Class persistentClass) {
        ...
        return new ShardedCriteriaImpl(new CriteriaId(nextCriteriaId++), shards,
                new CriteriaFactoryImpl(persistentClass), shardStrategy.getShardAccessStrategy());
    }
    ...
    public Criteria createCriteria(final String entityName) {
        ...
        return new ShardedCriteriaImpl(new CriteriaId(nextCriteriaId++), shards,
                new CriteriaFactoryImpl(entityName), shardStrategy.getShardAccessStrategy());
    }
    ...
    public Criteria createCriteria(final String entityName, final String alias) {
        ...
        return new ShardedCriteriaImpl(new CriteriaId(nextCriteriaId++), shards,
                new CriteriaFactoryImpl(entityName, alias), shardStrategy.getShardAccessStrategy());
    }

    @Override
    public Query createQuery(final String queryString) throws HibernateException {

        return new ShardedQueryImpl(new QueryId(nextQueryId++), shards, new AdHocQueryFactoryImpl(
                queryString), shardStrategy.getShardAccessStrategy());
    }
    ...
    public Query getNamedQuery(final String queryName) throws HibernateException {
        ...
        return new ShardedQueryImpl(new QueryId(nextQueryId++), shards, new NamedQueryFactoryImpl(
                queryName), shardStrategy.getShardAccessStrategy());
    }
    ...
    public String getEntityName(final Object object) throws HibernateException {
        ...
        ShardOperation<String> invoker = new ShardOperation<String>() {
            ...
            @Override
            public String execute(final Shard s) {
    ...
    public Filter enableFilter(final String filterName) {
        ...
        EnableFilterOpenSessionEvent event = new EnableFilterOpenSessionEvent(filterName);
        for (Shard shard : shards) {
            if (shard.getSession() != null) {
    ...
    public void disableFilter(final String filterName) {
        ...
        DisableFilterOpenSessionEvent event = new DisableFilterOpenSessionEvent(filterName);
        for (Shard shard : shards) {
            if (shard.getSession() != null) {
    ...
    public void setReadOnly(final Object entity, final boolean readOnly) {
        ...
        SetReadOnlyOpenSessionEvent event = new SetReadOnlyOpenSessionEvent(entity, readOnly);
        for (Shard shard : shards) {
            if (shard.getSession() != null) {
    ...
    public ShardId getShardIdForObject(final Object obj, final List<Shard> shardsToConsider) {
        ...
        if (shard == null) {
            ...
            if (obj instanceof HibernateProxy) {
                className = ((HibernateProxy) obj).getHibernateLazyInitializer()
                        .getPersistentClass().getName();
            } else {
                className = obj.getClass().getName();
            }
            IdentifierGenerator idGenerator = shard.getSessionFactoryImplementor()
                    .getIdentifierGenerator(className);
            if (idGenerator instanceof ShardEncodingIdentifierGenerator) {
                return ((ShardEncodingIdentifierGenerator) idGenerator)
                        .extractShardId(getIdentifier(obj));
            } else {
    ...
    List<ShardId> selectShardIdsFromShardResolutionStrategyData(
        ...
        IdentifierGenerator idGenerator = shardedSessionFactory.getIdentifierGenerator(srsd
                .getEntityName());
        if ((idGenerator instanceof ShardEncodingIdentifierGenerator) && (srsd.getId() != null)) {
            return Collections.singletonList(((ShardEncodingIdentifierGenerator) idGenerator)
                    .extractShardId(srsd.getId()));
        }
     ...
     */
    <T> T invokeOnShardWithObject(final ShardOperation<T> so, final Object object)
            throws HibernateException {
