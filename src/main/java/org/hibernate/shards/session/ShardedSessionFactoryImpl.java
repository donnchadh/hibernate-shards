 ...
 */
public class ShardedSessionFactoryImpl implements ShardedSessionFactoryImplementor,
    ...
    // map of SessionFactories used by this ShardedSessionFactory (might be a
    // subset of all SessionFactories)
    private final Map<SessionFactoryImplementor, Set<ShardId>> sessionFactoryShardIdMap;

    // map of all existing SessionFactories, used when creating a new
    // ShardedSessionFactory for some subset of shards
    ...
    // data to live in a single, well-known location (like distributed
    // sequences)
     ...
     * 
     * @param shardIds
     *            The ids of the shards with which this SessionFactory should be
     *            associated.
     * @param sessionFactoryShardIdMap
     *            Mapping of SessionFactories to shard ids. When using virtual
     *            shards, this map associates SessionFactories (physical shards)
     *            with virtual shards (shard ids). Map cannot be empty. Map keys
     *            cannot be null. Map values cannot be null or empty.
     * @param shardStrategyFactory
     *            factory that knows how to create the {@link ShardStrategy}
     *            that will be used for all shard-related operations
     * @param classesWithoutTopLevelSaveSupport
     *            All classes that cannot be saved as top-level objects
     * @param checkAllAssociatedObjectsForDifferentShards
     *            Flag that controls whether or not we do full cross-shard
     *            relationshp checking (very slow)
     */
    public ShardedSessionFactoryImpl(final List<ShardId> shardIds,
        ...
        for (Map.Entry<SessionFactoryImplementor, Set<ShardId>> entry : sessionFactoryShardIdMap
            ...
            for (ShardId shardId : shardIdSet) {
                // TODO(tomislav): we should change it so we specify control
                // shard in configuration
                if (shardId.getId() == CONTROL_SHARD_ID) {
                    controlSessionFactoryToSet = implementor;
                }
                if (!uniqueShardIds.add(shardId)) {
                    final String msg = String.format(
                            "Cannot have more than one shard with shard id %d.", shardId.getId());
                    log.error(msg);
    ...
    /**
     * Sets the {@link ControlSessionProvider} on id generators that implement
     * the {@link GeneratorRequiringControlSessionProvider} interface
     */
     ...
     * exposed on the public api we can't force clients to provide it. And at
     * any rate, exposing a ShardedConnection somewhat defeats the purpose of
     * tucking away all the sharding intelligence.
     */
    @Override
    ...
    public ShardedSession openSession(final Interceptor interceptor) throws HibernateException {
        ...
        return new ShardedSessionImpl(interceptor, this, shardStrategy,
                classesWithoutTopLevelSaveSupport, checkAllAssociatedObjectsForDifferentShards);
    }

    /**
     * Unsupported. This is a technical decision. See
     * {@link ShardedSessionFactoryImpl#openSession(Connection)} for an
     * explanation.
     */
    @Override
    ...
    public ShardedSession openSession() throws HibernateException {
        ...
        return new ShardedSessionImpl(this, shardStrategy, classesWithoutTopLevelSaveSupport,
                checkAllAssociatedObjectsForDifferentShards);
    }
    ...
    public boolean isClosed() {
        ...
        // a ShardedSessionFactory is closed if any of its SessionFactories are
        // closed
        for (SessionFactory sf : sessionFactories) {
            if (sf.isClosed()) {
                return true;
     ...
     * provide a ShardedSessionFactory that manages a subset of the
     * application's shards.
     * 
     * @param shardIds
     * @param shardStrategyFactory
    ...
    public ShardedSessionFactory getSessionFactory(final List<ShardId> shardIds,
        ...
        return new SubsetShardedSessionFactoryImpl(shardIds, fullSessionFactoryShardIdMap,
                shardStrategyFactory, classesWithoutTopLevelSaveSupport,
                checkAllAssociatedObjectsForDifferentShards);
    }
    ...
    @Override
    public Session openSession(final Connection connection,
            final boolean flushBeforeCompletionEnabled, final boolean autoCloseSessionEnabled,
